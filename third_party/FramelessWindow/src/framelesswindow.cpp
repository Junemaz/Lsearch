...
void FramelessWindow::moveWindow(QMouseEvent *event){
    ...
    if(event->buttons() == Qt::LeftButton){
        if(cursor().shape() == Qt::SizeHorCursor){
            int dx = event->pos().x() - localPos.x();
            QRect rect;
            if(mapToGlobal(event->pos()).x() < geometry().right() - 20){
                rect.setX(x() + dx);
                rect.setY(y());
            ...
            else if(mapToGlobal(event->pos()).x() > geometry().left() + 20){
                rect.setX(x());
                rect.setY(y());
                rect.setWidth(width() + dx);
                rect.setHeight(height());
            ...
            int dy = event->pos().y() - localPos.y();
            QRect rect;
            if(mapToGlobal(event->pos()).y() < geometry().bottom() - 20){
                rect.setX(x());
                rect.setY(y() + dy);
                rect.setWidth(width());
            ...
            else if(mapToGlobal(event->pos()).y() > geometry().top() + 20){
                rect.setX(x());
                rect.setY(y());
                rect.setWidth(width());
                rect.setHeight(height() + dy);
...
void FramelessWindow::hoverMove(QHoverEvent *event){
    ...
    if(mapToGlobal(event->pos()).x() == geometry().left() or mapToGlobal(event->pos()).x() == geometry().right()){
        setCursor(Qt::SizeHorCursor);
    }
    else if(mapToGlobal(event->pos()).y() == geometry().bottom() or
            mapToGlobal(event->pos()).y() == mapToGlobal(titlePoint).y()){
        setCursor(Qt::SizeVerCursor);
    }
    else if((mapToGlobal(event->pos()).x() > geometry().left() + resizeLookout and mapToGlobal(event->pos()).x() < geometry().right() - resizeLookout) and
            (mapToGlobal(event->pos()).y() > mapToGlobal(titlePoint).y() + resizeLookout and mapToGlobal(event->pos()).y() < frameGeometry().bottom() - resizeLookout)){
        unsetCursor();
    }
}
