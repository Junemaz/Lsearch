...
```
真实客户端转录已固化为回归：`tests/fixtures/mcp-trace-real.jsonl`（真实 opencode 会话，含
legacy `_meta.progressToken` 与 `notifications/cancelled`）：
```bash
./scripts/self-test-mcp-replay.sh -s tests/fixtures/mcp-trace-real.jsonl
```
