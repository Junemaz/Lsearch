...
class MainWindow : public QMainWindow {
 ...
 private:
  ...
  void toggleWindow() {
    // 外层窗口是 FramelessWindow（MainWindow 为其 central），统一操作顶层
    QWidget* top = window();
    if (top->isVisible()) { top->hide(); } else { top->showNormal(); top->raise(); top->activateWindow(); }
  }
...
int main(int argc, char** argv) {
  ...
  // ⚠️ ✕（FramelessWindow 的关闭按钮调用 close）→ 最后一个窗口关闭即退出
